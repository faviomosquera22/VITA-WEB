...
export default function StructuredHcePage() {
  ...
  const evolution = useMemo(
    () => (patient ? buildSignedEvolution(patient.medicalNotes) : []),
    [patient]
  );
  const examBySystem = useMemo(
    () => (patient ? buildExamBySystem(patient) : []),
    [patient]
  );
  ...
  return (
    ...
    >
      <PatientFinder
        ...
        title="Seleccion de paciente para HCE"
        subtitle="Busca por nombre, cedula o historia clinica y carga la HCE estructurada del paciente."
      />

      {patient ? <PatientContextSummary patient={patient} /> : null}
      ...
      {patient ? (
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <Panel title="Consulta actual" subtitle="Motivo de consulta y enfermedad actual en campos separados">
              <div className="space-y-2 text-xs text-slate-700">
                ...
                <Field label="Servicio de ingreso" value={getPatientServiceArea(patient)} />
              </div>
            </Panel>
        ...
        <Panel title="Sin paciente seleccionado">
          <p className="text-xs text-slate-500">Selecciona un paciente para visualizar su HCE estructurada.</p>
        </Panel>
      )}
...
function buildExamBySystem(patient: PatientRecord): ExamSystem[] {
  const latestVital = patient.vitalSigns[0];
  const taStatus =
    latestVital?.bloodPressure && latestVital.bloodPressure !== "120/80" ? "Alterado" : "Normal";
  ...
  return [
    ...
    {
      system: "Respiratorio",
      finding: `FR ${latestVital?.respiratoryRate ?? "-"} · SpO2 ${latestVital?.spo2 ?? "-"}%.`,
      status: spo2Status,
    },
    {
      system: "Neurologico",
    ...
    {
      system: "Abdominal",
      finding: "Sin hallazgos de alarma registrados en la evolucion actual.",
      status: "Normal",
    },
    {
      system: "Musculoesqueletico",
      finding: patient.nursingShiftReports[0]?.mobility || "Movilidad no registrada.",
      status: "Normal",
    },
    {
